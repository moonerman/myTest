...
- (void)initInfo
{
    ...
    [rightButton addTarget:self action:@selector(photoBtnOnClick) forControlEvents:UIControlEventTouchUpInside];
...
- (void)creatControl
{
    ...
    CGFloat marginY = (KMainH - scanW - padding - labelH - tabBarH) * 0.5;
    ...
    //扫描线
    UIImageView *line = [[UIImageView alloc] initWithFrame:CGRectMake(0, 10, scanW, 2)];
   // line.backgroundColor = [UIColor greenColor];
    ...
    //开启照明按钮
    UIButton *lightBtn = [[UIButton alloc] initWithFrame:CGRectMake(KMainW/2-50, CGRectGetMaxY(label.frame)+padding, 100, tabBarH)];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info
{
    ...
    UIView *scan = (UIView *)[self.view viewWithTag:100];
    scan.layer.contents = (id)image.CGImage;
    ...
    //    [self.session startRunning];
        ...
        if (features.count > 0) {
            ...
            [self showAlertWithTitle:@"没有识别到二维码或条形码" message:nil sureHandler:^{
                ZLog(@"点击了确认");
            } cancelHandler:nil];
